    ...
    navigator.geolocation.getCurrentPosition(position => {
      fetch(`https://nominatim.openstreetmap.org/reverse?lat=${position.coords.latitude}&lon=${position.coords.longitude}&format=json`)
        ...
        .then(data => {
          ...
          locationMessage.textContent = `Localização detectada: ${city}`;
          hideLoading();
        })
        .catch(() => {
          hideLoading();
...
function calcFrete() {
  const origin = document.getElementById('origin-city').value.trim().toLowerCase();
  if (!origin) {
  ...
  setTimeout(() => {
    hideLoading();
    // Valores fixos aproximados baseados em cidades comuns (simulação melhorada)
    const freightCosts = {
      'são paulo, sp': 0, // Mesma cidade
    ...
    const cost = freightCosts[origin] || 42.00; // Valor médio default
    alert(`Frete aproximado de ${origin.charAt(0).toUpperCase() + origin.slice(1)} para São Paulo: R$ ${cost.toFixed(2)}. Para envio exato, acesse o site dos Correios e use nossos dados como destinatário.`);
